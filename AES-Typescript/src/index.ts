...
import { ghash, polyval } from "./polyval.js";
import {
    Cipher,
    CipherWithOutput,
    clean,
...
export function expandKeyLE(key: Uint8Array): Uint32Array {
    ...
    if (![16, 24, 32].includes(len))
        throw new Error(
            `aes: wrong key size: should be 16, 24 or 32, got: ${len}`,
        );
    ...
    function gcm(key: Uint8Array, nonce: Uint8Array, AAD?: Uint8Array): Cipher {
        ...
        return {
            encrypt(plaintext: Uint8Array) {
                ...
                const toClean: (Uint8Array | Uint32Array)[] = [
                    xk,
                    authKey,
                    counter,
                    tagMask,
    ...
    (kek: Uint8Array): Cipher => ({
        encrypt(plaintext: Uint8Array) {
            ...
            if (plaintext.length === 8)
                throw new Error(
                    "8-byte keys not allowed in AESKW, use AESKWP instead",
                );
...
const stringToUint8Array = (string: string): Uint8Array => {
...
const plaintext = stringToUint8Array(message);
console.log("Message", message);
const ciphertext_ = ecb(key, {}).encrypt(plaintext);
console.log("Encode",uint8ArrayToString(ciphertext_));
const plaintext_ = ecb(key, {}).decrypt(ciphertext_);
console.log("Decode", uint8ArrayToString(plaintext_));
